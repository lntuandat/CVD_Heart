...
def manage_accounts():
    ...
    if request.method == 'POST' and 'add_patient' in request.form:
        ...
        if not is_valid_gmail(email):
            ...
            if cur.fetchone()['count'] > 0:
                ...
                email_body = f"""<div style='font-family:Arial,sans-serif;line-height:1.6'> <h2 style='color:#0d6efd'>Chào {ho_ten}</h2> <p>Bạn được bác sĩ thêm vào hệ thống CVD-App. Vui lòng xác nhận email để kích hoạt tài khoản.</p> <p>Mã xác thực của bạn: <strong style='font-size:1.2rem;'>{verification_code}</strong></p> <p>Nhấn vào liên kết sau để hoàn tất: <a href='{confirm_link}'>Xác nhận tài khoản</a></p> </div> """
                try:
    ...
    for p in raw_patients:
        ...
        if ngay_sinh and hasattr(ngay_sinh, "strftime"):
            ...
            ngay_sinh_val = ngay_sinh.strftime("%Y-%m-%d")
        else:
...
def export_diagnosis():
    ...
    input_data = [ ["Tuổi", data['age'], "Giới tính", data['gender']], ["BMI", data['bmi'], "Huyết áp (HATT/HATTr)", f"{data['systolic']}/{data['diastolic']}"], ["Cholesterol", data['cholesterol'], "Đường huyết", data['glucose']], ["Hút thuốc", "Có" if data['smoking']=="yes" else "Không", "Rượu/Bia", "Có" if data['alcohol']=="yes" else "Không"], ["Tập thể dục", "Có" if data['exercise']=="yes" else "Không", "", ""] ]
    ...
    for cell in ws[ws.max_row]:
        ...
        cell.border = border
        cell.alignment = center
    ws.append([ "Cao" if data['risk_level'] == 'high' else "Thấp", data['risk_percent'] + "%", "⚠️ Cần theo dõi" if data['risk_level'] == 'high' else "✅ Ổn định", "" ])
    for cell in ws[ws.max_row]:
        cell.font = normal_font
        cell.border = border
        cell.alignment = center
    ...
    advice_text = unescape(advice_text)
    ...
    if request.method == 'POST' and 'add_user' in request.form:
        ...
        if not is_valid_gmail(email):
            ...
            if cur.fetchone()['count'] > 0:
                ...
                token = generate_invite_token(payload)
                confirm_link = url_for('confirm_patient_invite', token=token, _external=True)
                email_body = f"""<div style='font-family:Arial,sans-serif;line-height:1.6'> <h2 style='color:#0d6efd'>Chào {ho_ten}</h2> <p>Bạn được quản trị viên thêm vào hệ thống CVD-App. Vui lòng xác nhận email để kích hoạt tài khoản.</p> <p>Mã xác thực của bạn: <strong style='font-size:1.2rem;'>{verification_code}</strong></p> <p>Nhấn vào liên kết sau để hoàn tất: <a href='{confirm_link}'>Xác nhận tài khoản</a></p> </div> """
                try:
                    send_email(email, "Xác nhận tài khoản CVD-App", email_body)
...
def export_admin_stats():
    ...
    border = Border( left=Side(style="thin", color="999999"), right=Side(style="thin", color="999999"), top=Side(style="thin", color="999999"), bottom=Side(style="thin", color="999999") )
    ws.merge_cells("A1:E1")
